...
function updateRateLimitDisplay(remaining) {
  const message = getTranslation("remainingGenerations").replace(
    "{count}",
    remaining,
  );
...
function displayCityAutocomplete(cities) {
  ...
  cityAutocomplete.innerHTML = cities
    .map((city) => {
...
cityInput.addEventListener("keydown", (e) => {
  ...
  const selectedItem = cityAutocomplete.querySelector(
...
function displayVideos(videos) {
  ...
  videos.forEach((video) => {
    ...
    card.className =
      "video-card bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition cursor-pointer";
